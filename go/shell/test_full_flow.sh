...
cleanup() {
    ...
    rm -f /tmp/np4_flow_*.log /tmp/np4_flow_*.fifo
}
trap cleanup EXIT
...
(tail -f /dev/null | "$BIN_DIR/np4cli" --port $NODE_A_PORT --bootstrap "$BOOTSTRAP_MULTIADDR" chat > /tmp/np4_flow_a.fifo 2>&1) &
cat /tmp/np4_flow_a.fifo > /tmp/np4_flow_a.log &
sleep 3

NODE_A_ID=$(grep "Peer ID:" /tmp/np4_flow_a.log | awk '{print $3}')
NODE_A_ADDR=$(grep "/ip4/127.0.0.1" /tmp/np4_flow_a.log | head -1 | awk '{print $1}')
green "A: $NODE_A_ID"
...
(tail -f /dev/null | "$BIN_DIR/np4cli" --port $NODE_B_PORT --bootstrap "$BOOTSTRAP_MULTIADDR" chat > /tmp/np4_flow_b.fifo 2>&1) &
cat /tmp/np4_flow_b.fifo > /tmp/np4_flow_b.log &
sleep 3

NODE_B_ID=$(grep "Peer ID:" /tmp/np4_flow_b.log | awk '{print $3}')
NODE_B_ADDR=$(grep "/ip4/127.0.0.1" /tmp/np4_flow_b.log | head -1 | awk '{print $1}')
green "B: $NODE_B_ID"
...
echo
echo "--- Step 5: B 连接 A ---"
OUTPUT=$("$BIN_DIR/np4cli" --port $NODE_B_PORT connect "$NODE_A_ADDR" 2>&1)
if echo "$OUTPUT" | grep -q "Connected"; then
    green "B -> A 连接成功"
    PASS=$((PASS+1))
else
...
"$BIN_DIR/np4cli" --port $NODE_A_PORT --bootstrap "$BOOTSTRAP_MULTIADDR" send --addr "$NODE_B_ADDR" "$NODE_B_ID" "hello-from-A" 2>&1
sleep 3
...
"$BIN_DIR/np4cli" --port $NODE_B_PORT --bootstrap "$BOOTSTRAP_MULTIADDR" send --addr "$NODE_A_ADDR" "$NODE_A_ID" "hello-from-B" 2>&1
sleep 3
